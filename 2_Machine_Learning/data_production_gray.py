...
lensed_images = np.array([ imageio.imread('/home/amitjit/output/spectrograms/PM/Lensed_' +str(i+1) + '.png', as_gray=True).ravel() for i in range(n_samples)])

lensed_labels = np.repeat("Lensed Wave", n_samples)
print('Finished reading lensed images.')

unlensed_images = np.array([ imageio.imread('/home/amitjit/output/spectrograms/Unlensed/Unlensed_' +str(i+1) + '.png', as_gray =True).ravel() for i in range(n_samples)])
...
np.savez('/home/amitjit/output/data_PM_gray', X_train = X_train, X_test = X_test, y_test = y_test, y_train = y_train, rank_test = rank_test)
